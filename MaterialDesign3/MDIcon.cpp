...
MDIcon::MDIcon(e3::Element* pParent)
	: MDIconBase(pParent)
{
	SetFont("MaterialDesign3/icons/MaterialIcons-Regular.ttf");
...
{
	{
		std::string cp = GetApplication()->GetAssetManager()->GetAssetPath("MaterialDesign3/icons/MaterialIcons-Regular.codepoints");
	...
	{
		std::string cp = GetApplication()->GetAssetManager()->GetAssetPath("MaterialDesign3/icons/MaterialIconsOutlined-Regular.codepoints");
		std::ifstream infile(cp);
		std::string name, code;
		while (infile >> name >> code)
		{
