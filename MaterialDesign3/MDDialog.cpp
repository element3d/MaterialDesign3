...
void MDDialog::Show()
{
	...
	mAnimation->Start(0.1, [this](float v) { SetOpacity(v); }, [this]() { mAnimation = nullptr; });
}
...
void MDDialog::Hide()
{
	if (!mAnimation)
		mAnimation = new e3::Animation();
	mAnimation->Start(0.1, [this](float v) { SetOpacity(1.0 - v); }, [this]() {
		SetVisibility(e3::EVisibility::Hidden);
		mOpened = false;
		mAnimation = nullptr; });
}
...
{
	if (mOpened)
		mBody->SetBackgroundColor(MDTheme::Get()->Surface);
	else
