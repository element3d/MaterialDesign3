#include "SelectionControlsPageBase.h"

#include <e3/ViewFactory.h>
#include "./MaterialDesign3TestValues.h"

SelectionControlsPageBase::SelectionControlsPageBase(e3::Element* pParent)
	: MDElement(pParent)
{
    mRadioGroup = new MDRadioGroup();
        this->SetOrientation((e3::EOrientation)1);
        this->SetOverflow((e3::EOverflow)2);
        this->SetAlignItemsHor((e3::EAlignment)2);
        this->SetAlignItemsVer((e3::EAlignment)0);
        this->SetPaddingBottom(e3::Dim("0", "0", "0", "50", "50", "50"));
    e3::Element* pElement1 = e3::ViewFactory::CreateShape( e3::EOrientation::Horizontal);
    AddElement(pElement1);
        pElement1->SetWidth("100%");
        pElement1->SetHeight(e3::Dim("0", "0", "0", "40", "40", "40"));
        pElement1->SetAlignItemsHor((e3::EAlignment)0);
        mBack = new MDIconButton();
        pElement1->AddElement(mBack);
        mBack->SetName(_s(arrow_back));
                MDScaffold* pCustomView1 = new MDScaffold();
        AddElement(pCustomView1);
        pCustomView1->SetMarginTop(e3::Dim("0", "0", "0", "50", "50", "50"));
        pCustomView1->SetWidth(e3::Dim("100%", "100%", "100%", "500", "500", "500"));
        pCustomView1->SetHeight(e3::Dim("100%", "100%", "100%", "700", "700", "700"));
        pCustomView1->SetOrientation((e3::EOrientation)1);
                MDAppBar* pCustomView2 = new MDAppBar();
        pCustomView1->AddElement(pCustomView2);
        pCustomView2->SetAlignItemsHor((e3::EAlignment)0);
                MDAppBarTitle* pCustomView3 = new MDAppBarTitle();
        pCustomView2->AddElement(pCustomView3);
        pCustomView3->SetText("Selection controls");
    e3::Element* pElement2 = e3::ViewFactory::CreateShape( e3::EOrientation::Horizontal);
    pCustomView1->AddElement(pElement2);
        pElement2->SetWidth("100%");
        pElement2->SetHeight("100%");
        pElement2->SetOrientation((e3::EOrientation)1);
        pElement2->SetScaling((e3::EScaling)1);
        e3::Text* pText1 = e3::ViewFactory::CreateText();
        pElement2->AddElement(pText1);
        pText1->SetText("MDCheckbox");
        pText1->SetFontSize("18dp");
        pText1->SetFontStyle((e3::EFontStyle)2);
        pText1->SetTextColor(MDTheme::Get()->OnSurface);
    e3::Element* pElement3 = e3::ViewFactory::CreateShape( e3::EOrientation::Horizontal);
    pElement2->AddElement(pElement3);
        pElement3->SetMarginTop("20dp");
    mCheckTest1 = e3::ViewFactory::CreateShape( e3::EOrientation::Horizontal);
    pElement3->AddElement(mCheckTest1);
        mCheckTest1->SetWidth("40dp");
        mCheckTest1->SetHeight("5dp");
        mCheckTest1->SetBackgroundColor(MDTheme::Get()->OnSurface);
        mCheckTest1->SetMarginRight("20dp");
    mCheckTest2 = e3::ViewFactory::CreateShape( e3::EOrientation::Horizontal);
    pElement3->AddElement(mCheckTest2);
        mCheckTest2->SetWidth("40dp");
        mCheckTest2->SetHeight("5dp");
        mCheckTest2->SetBackgroundColor(MDTheme::Get()->OnSurface);
        mCheckTest2->SetMarginRight("20dp");
    mCheckTest3 = e3::ViewFactory::CreateShape( e3::EOrientation::Horizontal);
    pElement3->AddElement(mCheckTest3);
        mCheckTest3->SetWidth("40dp");
        mCheckTest3->SetHeight("5dp");
        mCheckTest3->SetBackgroundColor(MDTheme::Get()->OnSurface);
    e3::Element* pElement4 = e3::ViewFactory::CreateShape( e3::EOrientation::Horizontal);
    pElement2->AddElement(pElement4);
        pElement4->SetMarginTop("20dp");
        mCheckbox1 = new MDCheckbox();
        pElement4->AddElement(mCheckbox1);
        mCheckbox1->SetMarginRight("20dp");
        mCheckbox2 = new MDCheckbox();
        pElement4->AddElement(mCheckbox2);
        mCheckbox2->SetMarginRight("20dp");
        mCheckbox3 = new MDCheckbox();
        pElement4->AddElement(mCheckbox3);
        e3::Text* pText2 = e3::ViewFactory::CreateText();
        pElement2->AddElement(pText2);
        pText2->SetText("MDRadio");
        pText2->SetMarginTop("40dp");
        pText2->SetFontSize("18dp");
        pText2->SetFontStyle((e3::EFontStyle)2);
        pText2->SetTextColor(MDTheme::Get()->OnSurface);
    e3::Element* pElement5 = e3::ViewFactory::CreateShape( e3::EOrientation::Horizontal);
    pElement2->AddElement(pElement5);
        pElement5->SetMarginTop("20dp");
    mRadioTest1 = e3::ViewFactory::CreateShape( e3::EOrientation::Horizontal);
    pElement5->AddElement(mRadioTest1);
        mRadioTest1->SetWidth("40dp");
        mRadioTest1->SetHeight("5dp");
        mRadioTest1->SetBackgroundColor(MDTheme::Get()->OnSurface);
        mRadioTest1->SetMarginRight("10dp");
    mRadioTest2 = e3::ViewFactory::CreateShape( e3::EOrientation::Horizontal);
    pElement5->AddElement(mRadioTest2);
        mRadioTest2->SetWidth("40dp");
        mRadioTest2->SetHeight("5dp");
        mRadioTest2->SetBackgroundColor(MDTheme::Get()->OnSurface);
        mRadioTest2->SetMarginRight("10dp");
    mRadioTest3 = e3::ViewFactory::CreateShape( e3::EOrientation::Horizontal);
    pElement5->AddElement(mRadioTest3);
        mRadioTest3->SetWidth("40dp");
        mRadioTest3->SetHeight("5dp");
        mRadioTest3->SetBackgroundColor(MDTheme::Get()->OnSurface);
    e3::Element* pElement6 = e3::ViewFactory::CreateShape( e3::EOrientation::Horizontal);
    pElement2->AddElement(pElement6);
        mRadio1 = new MDRadio();
        pElement6->AddElement(mRadio1);
        mRadio1->SetGroup(mRadioGroup);
        mRadio2 = new MDRadio();
        pElement6->AddElement(mRadio2);
        mRadio2->SetGroup(mRadioGroup);
        mRadio3 = new MDRadio();
        pElement6->AddElement(mRadio3);
        mRadio3->SetGroup(mRadioGroup);
        e3::Text* pText3 = e3::ViewFactory::CreateText();
        pElement2->AddElement(pText3);
        pText3->SetText("MDSwitch");
        pText3->SetMarginTop("40dp");
        pText3->SetFontSize("18dp");
        pText3->SetFontStyle((e3::EFontStyle)2);
        pText3->SetTextColor(MDTheme::Get()->OnSurface);
    e3::Element* pElement7 = e3::ViewFactory::CreateShape( e3::EOrientation::Horizontal);
    pElement2->AddElement(pElement7);
        pElement7->SetMarginTop("20dp");
    mSwitchTest1 = e3::ViewFactory::CreateShape( e3::EOrientation::Horizontal);
    pElement7->AddElement(mSwitchTest1);
        mSwitchTest1->SetWidth("40dp");
        mSwitchTest1->SetHeight("5dp");
        mSwitchTest1->SetBackgroundColor(MDTheme::Get()->OnSurface);
        mSwitch = new MDSwitch();
        pElement2->AddElement(mSwitch);
        mSwitch->SetMarginTop("20dp");

}
